...
class CheckDocumentExpiry extends Command
{
    protected $signature = 'employees:check-document-expiry
                            {--days=180 : Check documents expiring within X days}
                            {--test : Test mode - don\'t send actual notifications}';

    protected $description = 'Check labor card, Emirates ID, passport and residency (visa) expiry dates and notify HR before they expire';

    /**
     * document_type key => [EmployeeProfile date column, human label, EmployeeProfile number column or null]
     */
    protected array $documents = [
        'passport' => ['passport_expiry_date', 'Passport', 'passport_number'],
        'labor_card' => ['labor_card_expiry_date', 'Labor Card', 'labor_card_number'],
        'emirates_id' => ['emirates_id_expiry_date', 'Emirates ID', 'emirates_id_number'],
        'residency' => ['visa_validity', 'Residency Visa', null],
    ];
    ...
    public function handle()
    {
        ...
        $this->info("Checking documents expiring within {$daysThreshold} days...");
        ...
        foreach ($this->documents as $dateColumn => [$column, $label, $numberColumn]) {
        ...
        if (empty($tableData)) {
            $this->info("No documents expiring within {$daysThreshold} days.");
            return 0;
        }
