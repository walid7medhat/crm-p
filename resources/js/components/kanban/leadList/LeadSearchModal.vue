                    ...
                    <template v-for="field in section.fields" :key="field.id">
                        ...
                        <div class="col-md-6 mt-3">
                            ...
                            <v-select
                                ...
                                v-else-if="field.type === 'select' && field.id === 'property_type'"
                                ...
                                :clearable="hasValue(form.propertyType)"

                                append-to-body

                                class="custom-v-select"

                                @open="loadPropertyTypes"
                            ...
                            <v-select
                                ...
                                v-else-if="field.type === 'select' && field.id === 'stage'"
                                ...
                                :clearable="hasValue(form.stageId)"
                            ...
                            <v-select
                                ...
                                v-else-if="field.type === 'select' && field.id === 'lead_branch_source'"

                                v-model="form.branchSource"
                                ...
                                :clearable="hasValue(form.branchSource)"

                                append-to-body

                                class="custom-v-select"

                                @open="loadBranchSources"

                                :loading="loadingBranchSources"
...
// حالات التحميل لكل سيلكت

const loadingAreas = ref(false)

const loadingPropertyTypes = ref(false)

const loadingStages = ref(false)

const loadingResponsiblePersons = ref(false)

const loadingTeams = ref(false)

const loadingOffices = ref(false)

const loadingBranchSources = ref(false)



// دوال التحميل لكل سيلكت مع إمكانية Force Refresh

async function loadAreas(force = false) {

    if (!force && areaOptions.value.length > 0) return
...
async function loadPropertyTypes(force = false) {

    if (!force && propertyTypeOptions.value.length > 0) return
...
async function loadStages(force = false) {
    ...
    if (!force && stageOptions.value.length > 0) return
...
async function loadResponsiblePersons(force = false) {

    if (!force && allResponsiblePersons.value.length > 0 && !loadingResponsiblePersons.value) return

    loadingResponsiblePersons.value = true

    try {

        await fetchResponsiblePersonsWithFilter()
...
async function loadTeams(force = false) {
    ...
    if (!force && allTeams.value.length > 0 && !loadingTeams.value) return
...
async function loadOffices(force = false) {
    ...
    if (!force && officeOptions.value.length > 0) return
...
async function loadBranchSources(force = false) {

    if (!force && branchSourceOptions.value.length > 0) return

    loadingBranchSources.value = true

    try {

        const data = await fetchBranchSources()

        branchSourceOptions.value = data || []

    } catch (error) {

        console.error('Error loading branch sources:', error)

    } finally {

        loadingBranchSources.value = false
...
async function loadAllSelectDataForCurrentForm() {
    ...
    if (form.value.office && form.value.office.length > 0) {
        promises.push(loadOffices(true))
    }
    
    if (form.value.branchSource) {
        promises.push(loadBranchSources(true))
    }
...
function syncFormFromQuery(query) {
    ...
    nextTick(async () => {
        ...
        // تحميل بيانات السيلكتات التي لها قيم محددة
        await loadAllSelectDataForCurrentForm()
    })
}
...
const computedTeamOptions = computed(() => {
    ...
    filteredTeams.forEach((team) => {
        ...
        opts.push({
            ...
            role_name: team.role_name || 'Team',

            parent_name: team.parent_name || team.admin_parent_name || '',
...
onMounted(async () => {
    ...
    try {
        // تحميل البيانات الأساسية فقط - المكاتب والمراحل ومصادر الفروع
