<template>
    <div class="card basic-data-table">
        <div class="card-header">
            <h6 class="ui-h-mini card-title mb-0">Property Types List</h6>
        </div>
        <div class="card">
            <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-3"
                ...
                <div class="d-flex gap-2">
                    ...
                    <button v-if="this.$hasPermission('property_types-create')" 
                    ...
                    <div class="icon-field d-flex align-items-center" style="padding-bottom: 5px;">
                        ...
                        <div class="position-relative" style="width: 100%; max-width: 240px;">
                            <input type="text" class="form-control form-control-sm w-100 px-3 pe-5" v-model="searchText"
                                style="border-radius: 10px; height: 2.5rem;" placeholder="Search property types..." />
            ...
            <div class="card-body">
                    ...
                    <table class="table bordered-table mb-0">
                        ...
                        <tbody>
                            <tr v-for="propertyType in paginatedPropertyTypes" :key="propertyType.id">
                                ...
                                <td>
                                    <span v-if="propertyType.parent" class="badge bg-light text-dark">
                                        {{ propertyType.parent.name }}
                                    </span>
                                    <span v-else class="text-muted">Main Category</span>
                                </td>
                                <td>{{ formatDate(propertyType.created_at) }}</td>
                            
                                <td class="d-flex gap-2">
                                    <!-- Edit Button -->
                                    <a v-if="this.$hasPermission('property_types-edit')" href="javascript:void(0)"
                                    ...
                                    <!-- Delete Button -->
                                    <a v-if="this.$hasPermission('property_types-delete')" href="javascript:void(0)"
                ...
                <div v-if="loading" class="loading text-center py-4">
                    ...
                    <p class="mt-2">Loading property types...</p>
                </div>
                ...
                <div v-if="!loading && propertyTypes.length === 0" class="no-data text-center py-4">
                    ...
                    <button v-if="this.$hasPermission('property_types-create')" class="btn btn-primary mt-2" @click="addPropertyType">
                        <iconify-icon icon="lucide:plus" class="me-2"></iconify-icon>
...
export default {
    ...
    methods: {
        hasAnyPropertyTypePermission() {
            return this.$hasPermission('property_types-edit') || 
                   this.$hasPermission('property_types-delete');
        },
        ...
        async fetchPropertyTypes() {
            try {
                ...
                const response = await fetch(API_ENDPOINTS.PROPERTY_TYPES, {
                    method: 'GET',
                    headers: {
                ...
                console.log('Property types loaded:', this.propertyTypes);
                
            } catch (error) {
                console.error('Error fetching property types:', error);
                this.propertyTypes = [];
                
                this.showNotification('Failed to load property types. Please try again.', 'error');
            } finally {
                this.loading = false;
            }
        },
        ...
        // Action methods
        addPropertyType() {
            if (!this.$hasPermission('property_types-create')) {
                this.showNotification('You do not have permission to create property types', 'warning');
                return;
            }
            this.$router.push('/add-property_type');
        },

        editPropertyType(typeId) {
            if (!this.$hasPermission('property_types-edit')) {
                this.showNotification('You do not have permission to edit property types', 'warning');
                return;
            }
            this.$router.push(`/property_types/${typeId}/edit`);
        },

        async deletePropertyType(propertyType) {
            if (!this.$hasPermission('property_types-delete')) {
                this.showNotification('You do not have permission to delete property types', 'warning');
                return;
            }
            ...
            if (confirmed) {
                try {
                    this.loading = true;
                    const token = localStorage.getItem('token');
                    const response = await fetch(API_ENDPOINTS.PROPERTY_TYPE_BY_ID(propertyType.id), {
                        method: 'DELETE',
                        headers: {
                            'Authorization': 'Bearer ' + token,
                            'Content-Type': 'application/json'
                    ...
                    this.showNotification(`Failed to delete property type: ${error.message}`, 'error');
                } finally {
                    this.loading = false;
                }
            }
