<template>
    <div class="card basic-data-table">
        <div class="card-header">
            <h6 class="ui-h-mini card-title mb-0">Unit Views List</h6>
        </div>
        <div class="card">
            <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-3"
                ...
                <div class="d-flex gap-2">
                    <!-- Add Unit View Button -->
                    <button v-if="this.$hasPermission('unit_views-create')" 
                    ...
                    <div class="icon-field d-flex align-items-center" style="padding-bottom: 5px;">
                        ...
                        <div class="position-relative" style="width: 100%; max-width: 240px;">
                            <input type="text" class="form-control form-control-sm w-100 px-3 pe-5" v-model="searchText"
                                style="border-radius: 10px; height: 2.5rem;" placeholder="Search Unit Views..." />
            ...
            <div class="card-body">
                        ...
                        <tbody>
                            <tr v-for="propertyType in paginatedPropertyTypes" :key="propertyType.id">
                                ...
                                <td class="d-flex gap-2">
                                    <!-- Edit Button -->
                                    <a v-if="this.$hasPermission('unit_views-edit')" href="javascript:void(0)"
                                        class="w-32-px h-32-px bg-success-focus text-success-main rounded-circle d-inline-flex align-items-center justify-content-center"
                                        @click="editPropertyType(propertyType.id)"
                                        title="Edit Unit View">
                                        <iconify-icon icon="lucide:edit"></iconify-icon>
                                    </a>

                                    <!-- Delete Button -->
                                    <a v-if="this.$hasPermission('unit_views-delete')" href="javascript:void(0)"
                                        class="w-32-px h-32-px bg-danger-focus text-danger-main rounded-circle d-inline-flex align-items-center justify-content-center"
                                        @click="deletePropertyType(propertyType)"
                                        title="Delete Unit View">
                ...
                <div v-if="loading" class="loading text-center py-4">
                    ...
                    <p class="mt-2">Loading Unit Views...</p>
                </div>
                ...
                <div v-if="!loading && propertyTypes.length === 0" class="no-data text-center py-4">
                    ...
                    <p>No Unit Views found</p>
                    <button v-if="this.$hasPermission('unit_views-create')" class="btn btn-primary mt-2" @click="addPropertyType">
                        <iconify-icon icon="lucide:plus" class="me-2"></iconify-icon>
                        Add First Unit View
                    </button>
                </div>
...
export default {
    ...
    methods: {
        hasAnyPropertyTypePermission() {
            return this.$hasPermission('unit_views-edit') || 
                   this.$hasPermission('unit_views-delete');
        },
        ...
        async fetchPropertyTypes() {
            try {
                ...
                const response = await fetch(API_ENDPOINTS.UNIT_VIEWS, {
                    method: 'GET',
                    headers: {
                ...
                console.log('Unit Views loaded:', this.propertyTypes);
                
            } catch (error) {
                console.error('Error fetching Unit Views:', error);
                this.propertyTypes = [];
                
                this.showNotification('Failed to load Unit Views. Please try again.', 'error');
            } finally {
                this.loading = false;
            }
        },
        ...
        // Action methods
        addPropertyType() {
            if (!this.$hasPermission('unit_views-create')) {
                this.showNotification('You do not have permission to create Unit Views', 'warning');
                return;
            }
            this.$router.push('/add-unit_view');
        },

        editPropertyType(typeId) {
            if (!this.$hasPermission('unit_views-edit')) {
                this.showNotification('You do not have permission to edit Unit Views', 'warning');
                return;
            }
            this.$router.push(`/unit_views/${typeId}/edit`);
        },

        async deletePropertyType(propertyType) {
            if (!this.$hasPermission('unit_views-delete')) {
                this.showNotification('You do not have permission to delete Unit Views', 'warning');
                return;
            }
            ...
            if (confirmed) {
                try {
                    this.loading = true;
                    const token = localStorage.getItem('token');
                    const response = await fetch(API_ENDPOINTS.UNIT_VIEW_BY_ID(propertyType.id), {
                        method: 'DELETE',
                        headers: {
                            'Authorization': 'Bearer ' + token,
                            'Content-Type': 'application/json'
                    ...
                    if (response.ok) {
                        ...
                        throw new Error(errorData.message || 'Failed to delete Unit View');
                    }
                } catch (error) {
                    console.error('Error deleting Unit View:', error);
                    this.showNotification(`Failed to delete Unit View: ${error.message}`, 'error');
                } finally {
                    this.loading = false;
                }
            }
